...
export function ProductCard({product,variant='default'}:{product:Product;variant?:'default'|'catalog'}){
  ...
  if(variant==='catalog')return <article className="group border-t border-slate-300 pt-4">
    <Link to={detailUrl} className="relative block aspect-[4/3] overflow-hidden bg-canvas">
      <ProductImageScroller product={product} alt={zh?`${product.nameZh}产品图片`:`${product.nameEn} product image`} className="size-full" imageClassName="object-cover transition duration-500 ease-out group-hover:scale-[1.018]"/>
    ...
    <div className="pt-5">
      ...
      <div className="mt-5 flex items-center justify-between gap-4 border-t border-slate-200 pt-4 text-sm font-bold">
        <Link to={detailUrl} className="inline-flex min-h-11 items-center gap-2 text-ink transition-colors hover:text-accent">{zh?'查看产品详情':'View product details'}<ArrowRight size={15}/></Link>
  ...
  return <article className="group overflow-hidden border border-line bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lift">
    <Link to={detailUrl} className="block aspect-[4/3] overflow-hidden"><ProductImageScroller product={product} alt={zh?`${product.nameZh}产品图片`:`${product.nameEn} product image`} className="size-full" imageClassName="object-cover transition duration-500 group-hover:scale-105"/></Link>
    <div className="p-6">
