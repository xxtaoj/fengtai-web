...
export function BaseInquiryForm({kind}:{kind:Kind}){
  ...
  return <form onSubmit={submit} noValidate className="grid gap-5 rounded-sm border border-line bg-white p-5 shadow-sm md:grid-cols-2 md:p-8">
    ...
    {kind==='contact'&&<FormField label={zh?'询盘类型':'Inquiry Type'}><SelectInput name="type"><option>{zh?'产品询盘':'Product inquiry'}</option><option>{zh?'外贸合作':'Export cooperation'}</option><option>{zh?'国内采购':'Domestic purchase'}</option><option>{zh?'提交订单':'Submit an order'}</option><option>{zh?'工厂参观':'Factory visit'}</option><option>{zh?'供应商合作':'Supplier cooperation'}</option><option>{zh?'其他':'Other'}</option></SelectInput></FormField>}
    <FormField label={zh?'产品':'Product'} required error={errors.product}><TextInput name="product" placeholder="[主营产品]"/></FormField>
    {kind==='export'&&<>
    ...
    <div className="md:col-span-2"><FormField label={zh?'留言与产品规格':'Message & Specifications'} required error={errors.message}><TextArea name="message"/></FormField></div>
